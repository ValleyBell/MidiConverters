...
UINT16 ReadBankOffset(UINT8* Buffer);
	...
	*TempPnt = 0x00;
	...
	else
	{
		...
		if (! FileCount)
		{
			...
			CurFile = 0x00;
			CurPos = SongPos;
			OutLen = 0xC000 | ReadBankOffset(&InData[SongPos]);
			while(CurPos < OutLen)
			{
				TempLng = 0xC000 | ReadBankOffset(&InData[CurPos]);
				if (! (InData[CurPos + 0x01] & 0x80))
					break;
				if (TempLng < OutLen)
					OutLen = TempLng;
...
UINT8 DataEast2Mid(UINT32 InLen, UINT8* InData, UINT32 InAddr, UINT32* OutLen, UINT8** OutData)
{
	...
	*OutData = (UINT8*)malloc(*OutLen);
	...
	WriteBigEndianS(&DstData[DstPos + 0x02], 0x0004);	// Tracks: 4
	WriteBigEndianS(&DstData[DstPos + 0x04], 0x0008);	// Ticks per Quarter: 0x08
	DstPos += 0x06;
	
	for (CurTrk = 0x00; CurTrk < 0x04; CurTrk ++)
	{
		InPos = 0xC000 | ReadBankOffset(&InData[InAddr + CurTrk * 0x04]);
		MidChn = (CurTrk == 0x03) ? 0x09 : CurTrk;
		...
		DstData[DstPos + 0x02] = InData[InAddr + 0x12] * 0x08;
		...
		while(! TrkEnd)
		{
			if (! (InData[InPos] & 0x80) || InData[InPos] == 0x88)
			{
				...
				if ((PitchToNote != 0xFF || (NoAtk & 0x01)) && CurNote < 0x80)
				{
					NoteDiff = 0x00;
					if (PitchToNote != 0xFF)
					{
				...
				else
				{
					...
					if (CurNote < 0x80)
					{
						if (LastNote < 0x80 && CurNote != LastNote)
						{
				...
				if (PitchToNote != 0xFF && CurNote == 0x80)
				{
